...
/**
 * GET /api/startups/[id]/preview/[[...path]] - Serve files for preview
 */
export async function GET(
  ...
  try {
    const { id: startupId, path: urlPath = [] } = await params;
    ...
    const fullPath = path.join(startupPath, filePathStr);

    // Security check: ensure file is within startup directory
    const resolvedPath = path.resolve(fullPath);
    const resolvedStartupPath = path.resolve(startupPath);

    if (!resolvedPath.startsWith(resolvedStartupPath)) {
